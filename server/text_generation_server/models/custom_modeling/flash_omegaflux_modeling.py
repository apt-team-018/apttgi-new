...
class OmegaFluxRotary(PositionRotaryEmbedding):
    def forward(
        self,
        query: torch.Tensor,
        key: torch.Tensor,
...
class OmegaFluxLayerNorm(nn.Module):
...
class FlashOmegaFluxAttention(torch.nn.Module):
    def __init__(
        self,
        prefix: str,
        config,
        ...
        self.rotary_emb = OmegaFluxRotary.static(
            config=config,
            dim=self.head_size,
        ...
        if self.use_qk_norm:
            self.q_norm = OmegaFluxLayerNorm(
                prefix=f"{prefix}.q_norm",
                weights=weights,
                eps=config.layer_norm_eps,
            )
            self.k_norm = OmegaFluxLayerNorm(
...
class FlashOmegaFluxLayer(nn.Module):
    def __init__(self, layer_id, config, weights):
        super().__init__()
        prefix = f"model.layers.{layer_id}"
        self.self_attn = FlashOmegaFluxAttention(
            prefix=f"{prefix}.self_attn", config=config, weights=weights
        )
        self.mlp = OmegaFluxMLP(prefix=f"{prefix}.mlp", config=config, weights=weights)
...
class FlashOmegaFluxModel(torch.nn.Module):
...
class FlashOmegaFluxForCausalLM(torch.nn.Module):
    def __init__(self, config, weights):
        super().__init__()

        self.model = FlashOmegaFluxModel(config, weights)
        try:
