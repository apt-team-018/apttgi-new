...
class OmegaMaizeConfig(PretrainedConfig):
    model_type = "omegamaize"
...
class OmegaMaizeAttention(torch.nn.Module):
    def __init__(
        self,
        prefix: str,
        config,
...
class OmegaMaizeLayer(nn.Module):
    def __init__(self, prefix, config, weights):
        super().__init__()
        self.self_attn = OmegaMaizeAttention(
            prefix=f"{prefix}.self_attn", config=config, weights=weights
        )
        self.mlp = OmegaMaizeMLP(prefix=f"{prefix}.mlp", config=config, weights=weights)
...
class OmegaMaizeModel(torch.nn.Module):
    def __init__(self, prefix, config, weights):
        super().__init__()
...
class FlashOmegaMaizeForCausalLM(torch.nn.Module):
