...
class OmegaIrisVisionEmbeddings(nn.Module):
    """
...
class OmegaIrisVisionAttention(nn.Module):
    def __init__(self, prefix, config, weights):
        super().__init__()
        self.config = config
        self.embed_dim = config.hidden_size
...
class OmegaIrisVisionMLP(nn.Module):
    def __init__(self, prefix, config, weights):
        super().__init__()
        self.config = config
...
class OmegaIrisEncoderLayer(nn.Module):
    def __init__(self, prefix, config, weights):
        super().__init__()
        self.embed_dim = config.hidden_size
        self.self_attn = OmegaIrisVisionAttention(
...
class OmegaIrisEncoder(nn.Module):
    def __init__(self, prefix, config, weights):
        super().__init__()
        self.config = config
        self.layers = nn.ModuleList(
            [
                OmegaIrisEncoderLayer(
...
class OmegaIrisVisionTransformer(nn.Module):
    def __init__(self, prefix, config, weights):
        super().__init__()
        self.config = config
        self.embeddings = OmegaIrisVisionEmbeddings(
            prefix=f"{prefix}.embeddings", config=config, weights=weights
        )
        self.encoder = OmegaIrisEncoder(
...
class OmegaIrisRMSNorm(nn.Module):
    def __init__(self, prefix, weights, eps):
        """
        OmegaIrisRMSNorm is equivalent to T5LayerNorm
        """
        super().__init__()
...
class OmegaIrisPerceiverAttention(nn.Module):
    def __init__(self, prefix, config, weights):
        super().__init__()
...
class OmegaIrisPerceiverLayer(nn.Module):
    def __init__(self, prefix, config, weights):
        super().__init__()
        self.hidden_size = config.text_config.hidden_size
        ...
        self.input_latents_norm = OmegaIrisRMSNorm(
            prefix=f"{prefix}.input_latents_norm",
            weights=weights,
            eps=self.rms_norm_eps,
        )
        self.input_context_norm = OmegaIrisRMSNorm(
            prefix=f"{prefix}.input_context_norm",
            weights=weights,
            eps=self.rms_norm_eps,
        )
        self.self_attn = OmegaIrisPerceiverAttention(
            prefix=f"{prefix}.self_attn", config=config, weights=weights
        )
        self.post_attention_layernorm = OmegaIrisRMSNorm(
            prefix=f"{prefix}.post_attention_layernorm",
            weights=weights,
            eps=self.rms_norm_eps,
        )
        self.mlp = OmegaIrisMLP(prefix=f"{prefix}.mlp", config=config, weights=weights)
...
class OmegaIrisPerceiverResampler(nn.Module):
...
class OmegaIrisConnector(nn.Module):
    def __init__(self, prefix, config, weights):
        super().__init__()
        self.modality_projection = OmegaIrisMLP(
            prefix=f"{prefix}.modality_projection", config=config, weights=weights
        )
        self.perceiver_resampler = OmegaIrisPerceiverResampler(
...
class OmegaIrisForConditionalGeneration(nn.Module):
    def __init__(self, prefix, config, weights):
        super().__init__()
        ...
        self.vision_model = OmegaIrisVisionTransformer(
            prefix=f"{prefix}.model.vision_model" if prefix else "model.vision_model",
            config=vision_config,
            weights=weights,
        )
        self.connector = OmegaIrisConnector(
